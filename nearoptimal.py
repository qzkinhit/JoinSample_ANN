...
class MultiDimHash(object):
    ...
    def _initializeGrids(self):
        ...
        radius_offset = sp.ones(self.dim) * self.radius
        self.gridBalls = sp.random.random((int(self._findAmountOfGrids()), self.dim))
    ...
    def _initializeProjection(self):
        if self.dim <= self.lowerDimensionBound:
            ...
            self.projection = sp.random.standard_normal(projection_shape)
    ...
    def _findHypercube(self, point):
        ...
        return np.array(hypercube_indices, dtype=int), np.array(relative_point)
    ...
    def _findLocalBall_noinline(self, point):
        ...
        for i, ball in enumerate(self.gridBalls):
            distance = point - ball
            if sp.dot(distance.T, distance) <= self.radiusSquared:
                return i
    ...
    def insert(self, point, satellite):
        ...
        """
        point = sp.dot(self.projection, point)
    ...
    def knn(self, point, k):
        ...
        def sortKey(xxx_todo_changeme):
            ...
            return - sp.dot(distance.T, distance)
